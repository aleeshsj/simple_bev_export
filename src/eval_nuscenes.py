...
def run_model(model, loss_fn, d, device='cuda:0', sw=None,export=False,onnx_runtime=False):
    metrics = {}
    ...
    if export:
        ...
        model_path="/media/bmw/aleesha/cluster/myrun/baseline/Export/artifacts/simple_bev.onnx"
        ...
        if onnx_runtime:
            ...
            inputs_ = {
            "rgb_camXs": rgb_camXs.cpu().numpy(),
            "pix_T_cams": pix_T_cams.cpu().numpy(),
            "cam0_T_camXs": cam0_T_camXs.cpu().numpy(),
            }
           
            import torch.nn as nn

            _, feat_bev_e_onnx, seg_bev_e_onnx, center_bev_e_onnx, offset_bev_e_onnx = sess.run(None, inputs_)

            # import torch
            import torch.nn as nn
            print("#"*80)
            # Example tensors
            print("MSE ONNX VS Pytorch ")
            print("#"*80,"\n")

            # MSE loss
            mse = nn.functional.mse_loss(feat_bev_e.cpu(), torch.from_numpy(feat_bev_e_onnx).cpu())
            print("feat_bev_e MSE:", mse.item(),"\n")

            mse = nn.functional.mse_loss(seg_bev_e.cpu(), torch.from_numpy(seg_bev_e_onnx).cpu())
            print("seg_bev_e MSE:", mse.item(),"\n")

            mse = nn.functional.mse_loss(center_bev_e.cpu(), torch.from_numpy(center_bev_e_onnx).cpu())
            print("center_bev_e MSE:", mse.item(),"\n")

            mse = nn.functional.mse_loss(offset_bev_e.cpu(), torch.from_numpy(offset_bev_e_onnx).cpu())
            print("offset_bev_e MSE:", mse.item(),"\n")
...
def main(
    ...
    # change
    # 448x800
    # final_dim = (int(448 * res_scale), int(800 * res_scale))
    ...
    while global_step < max_iters:
        ...
        with torch.no_grad():
            if export:
                 run_model(model, seg_loss_fn, sample, device, sw_ev,export=True)
                 exit()
            elif onnx_runtime:
                run_model(model, seg_loss_fn, sample, device, sw_ev,onnx_runtime=True)
                total_loss, metrics = run_model(model, seg_loss_fn, sample, device,)# sw_ev,onnx_runtime=False)
